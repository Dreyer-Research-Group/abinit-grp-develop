...
public :: hartremet         ! AMSrev: version for metric perturbation
...
!CEDrev: G0
subroutine hartremet(cplexin,gmet,gprimd,qphon,gsqcut,idir,ipert,mpi_enreg,natom,nfft,ngfft,&
&  rhog,vhartr1)
...
!Arguments ------------------------------------
!scalars
 integer,intent(in) :: cplexin,idir,ipert,natom,nfft !CEDrev cplexin
 real(dp),intent(in) :: gsqcut
 type(MPI_type),intent(in) :: mpi_enreg
!arrays
 integer,intent(in) :: ngfft(18)
 real(dp),intent(in) :: gmet(3,3),gprimd(3,3),qphon(3),rhog(2,nfft)
 !CEDrev:
 real(dp),intent(out) :: vhartr1(mod(cplexin,10)*nfft)
...
!CEDrev:
integer :: excludeg0,nog0,cplex,alpha,gamma
real(dp) :: term_ced,gq2_ced,gqc(3),qpc(3),g2,a_idir ! Try myself

! *************************************************************************

! AMSrev[
 write(*,*) 'ams: hartremet: I am in'
 if( .not. (ipert==natom+6))then
   write(message, '(a,i0,a,a)' )&
&   'From the calling routine, ipert=',ipert,ch10,&
&   'so this routine for the metric perturbation should not be called.'
   ABI_BUG(message)
 end if
! AMSrev ]
...
! AMScom: the following checks are copied from hartre.F90
! I think some similar check must be perfomed here because 
! we have to avoid the G=0 at the denominator
! Indeed! but chere we are checking q!=0, NOT G  !!! so maybe unecessary..
!! qeq05=0
!! if (qeq0==0) then
...
!CEDrev: From MS
!AMScom: in this subroutine there is no dependence by q. This is a difference
!        respect to hartre.F90 
 nog0=0
 if ( excludeg0==1) nog0=1
...
!      of the inner loop
       ...
       !CEDrev: G0
       !if(i23==0  .and. ig2==0 .and. ig3==0)then
       if((i23==0  .and. ig2==0 .and. ig3==0).or.&
&         (i23==0 .and. nog0==1  .and. ig2==0 .and. ig3==0))then
         ii1=2
         work1(re,1+i23)=0.0_dp
         work1(im,1+i23)=0.0_dp
       end if
...
! AMSrev[
           gqr(1)=gq(1,i1)
! get |G+q|**2
           gq2= (gqr(1)+qphon(1))*gmet(1,1)*(gqr(1)+qphon(1))+&
&               (gqr(2)+qphon(2))*gmet(2,2)*(gqr(2)+qphon(2))+&
&               (gqr(3)+qphon(3))*gmet(3,3)*(gqr(3)+qphon(3))+&
&           2.0_dp*((gqr(1)+qphon(1))*gmet(1,2)*(gqr(2)+qphon(2))+&
&                   (gqr(1)+qphon(1))*gmet(1,3)*(gqr(3)+qphon(3))+&
&                   (gqr(2)+qphon(2))*gmet(2,3)*(gqr(3)+qphon(3)))

! CEDrev TEST: Jesus Andrea WTF is this?
!if (dot_product(qphon,qphon) > 1e-10) then
!   write(*,'(a5,7e12.4e2)') 'GQ2',gqr(:),qphon(:),gq2
! Looks like gq2 is ok, missing a (2\pi)^2
!end if

!           term=((gqr(1)+qphon(1))*gmet(1,1)*gqr(1)+(gqr(2)+qphon(2))*gmet(2,2)*gqr(2)+&
!&                (gqr(3)+qphon(3))*gmet(3,3)*gqr(3)+(gqr(1)+qphon(1))*gmet(1,2)*gqr(2)+&
!&                (gqr(1)+qphon(1))*gmet(1,3)*gqr(3)+(gqr(2)+qphon(2))*gmet(2,1)*gqr(1)+&
!&                (gqr(2)+qphon(2))*gmet(2,3)*gqr(3)+(gqr(3)+qphon(3))*gmet(3,1)*gqr(1)+&
!&                (gqr(3)+qphon(3))*gmet(3,2)*gqr(2))*qphon(idir)-&
!&               ((gqr(1)+qphon(1))*gmet(1,1)*qphon(1)+(gqr(2)+qphon(2))*gmet(2,2)*qphon(2)+&
!&                (gqr(3)+qphon(3))*gmet(3,3)*qphon(3)+(gqr(1)+qphon(1))*gmet(1,2)*qphon(2)+&
!&                (gqr(1)+qphon(1))*gmet(1,3)*qphon(3)+(gqr(2)+qphon(2))*gmet(2,1)*qphon(1)+&
!&                (gqr(2)+qphon(2))*gmet(2,3)*qphon(3)+(gqr(3)+qphon(3))*gmet(3,1)*qphon(1)+&
!&                (gqr(3)+qphon(3))*gmet(3,2)*qphon(2))*gqr(idir)-&
!&               (gqr(1)*gmet(1,1)*qphon(1)+gqr(2)*gmet(2,2)*qphon(2)+gqr(3)*gmet(3,3)*qphon(3)+&
!&                gqr(1)*gmet(1,2)*qphon(2)+gqr(1)*gmet(1,3)*qphon(3)+gqr(2)*gmet(2,1)*qphon(1)+&
!&                gqr(2)*gmet(2,3)*qphon(3)+gqr(3)*gmet(3,1)*qphon(1)+gqr(3)*gmet(3,2)*qphon(2))*(gqr(idir)+qphon(idir))

           if(idir.eq.1)then
             term=&
&             (gqr(1)+qphon(1))*qphon(2)*(-gqr(2))+(gqr(1)+qphon(1))*qphon(3)*(-gqr(3))+&
&             (gqr(2)+qphon(2))*qphon(1)*(gqr(2))+(gqr(3)+qphon(3))*qphon(1)*(gqr(3))+&
&             (gqr(1)+qphon(1))*qphon(1)*(-gqr(1))+(gqr(2)+qphon(2))*qphon(2)*(-gqr(1))+(gqr(3)+qphon(3))*qphon(3)*(-gqr(1))
           else if(idir.eq.2)then
             term=&
&             (gqr(1)+qphon(1))*qphon(2)*(gqr(1))+(gqr(2)+qphon(2))*qphon(1)*(-gqr(1))+&
&             (gqr(2)+qphon(2))*qphon(3)*(-gqr(3))+(gqr(3)+qphon(3))*qphon(2)*(gqr(3))+&
&             (gqr(1)+qphon(1))*qphon(1)*(-gqr(2))+(gqr(2)+qphon(2))*qphon(2)*(-gqr(2))+(gqr(3)+qphon(3))*qphon(3)*(-gqr(2))
           else if(idir.eq.3)then
             term=&
&             (gqr(1)+qphon(1))*qphon(3)*(gqr(1))+(gqr(2)+qphon(2))*qphon(3)*(gqr(2))+&
&             (gqr(3)+qphon(3))*qphon(1)*(-gqr(1))+(gqr(3)+qphon(3))*qphon(2)*(-gqr(2))+&
&             (gqr(1)+qphon(1))*qphon(1)*(-gqr(3))+(gqr(2)+qphon(2))*qphon(2)*(-gqr(3))+(gqr(3)+qphon(3))*qphon(3)*(-gqr(3))
           end if



! CEDrev: Commented out for testing
           term=-piinv*term/gq2/((gqr(1)**2+(gqr(2))**2+(gqr(3))**2))!gs
!           term=-piinv*term/gq2/gs

! CEDrev: I am confused about the mess that Andrea wrote. I will try
! it myself here We would like to get -(G^\beta+q^\beta) / |G+q|^2 +
! G^\beta / G^2 (Eq. 28 in 1811.12893v1). Not sure about the factors of
! 2\pi (whether they should be inculded) so in full Cartesian for now

!qpc(:)=two_pi*matmul(gprimd(:,:),qphon(:)) ! Cartesian q point
!gqc(:)=two_pi*matmul(gprimd(:,:),gqr(:)) ! Cartesian G
!gq2=dot_product(gqc(:)+qpc(:),gqc(:)+qpc(:)) ! |G+q|**2
!term_ced=four_pi*(-(gqc(idir)+qpc(idir))/gq2 + gqc(idir)/dot_product(gqc(:),gqc(:)))

! So far I have not changed anything. Confirmed that gq2 is correct,
! albeit w/out the (2\pi)^2 The numerator matches what is in Max's
! notes but does not seem to match the paper??? If we take the
! expression in the notes as correct, then we have a strange
! combination of cartesian (gq2) and not (everything else).

! Lets go with Eq. 71/126 in Max's May 2017 notes, but do it all in cartesian
qpc(:)=two_pi*matmul(gprimd(:,:),qphon(:)) ! Cartesian q point
gqc(:)=two_pi*matmul(gprimd(:,:),gqr(:)) ! Cartesian G
gq2=dot_product(gqc(:)+qpc(:),gqc(:)+qpc(:)) ! |G+q|**2
g2=dot_product(gqc(:),gqc(:)) ! |G|**2 

! Looks like we need to have a factor of a/pi where a in in idir
if (idir==1) then
   a_idir=sqrt(dot_product(matmul(gprimd(:,:),(/ 1, 0, 0 /)), & 
        & matmul(gprimd(:,:),(/ 1, 0, 0 /))))*pi
elseif (idir==2) then
   a_idir=sqrt(dot_product(matmul(gprimd(:,:),(/ 0, 1, 0 /)), & 
        & matmul(gprimd(:,:),(/ 0, 1, 0 /))))*pi
elseif (idir==3) then
   a_idir=sqrt(dot_product(matmul(gprimd(:,:),(/ 0, 0, 1 /)), & 
        & matmul(gprimd(:,:),(/ 0, 0, 1 /))))*pi
end if
...
do alpha=1,3
   do gamma=1,3
      ...
      if (gamma==idir) then
         term_ced=term_ced+(gqc(alpha)+qpc(alpha))*qpc(gamma)*gqc(alpha)
      end if
      if (alpha==idir) then
         term_ced=term_ced-(gqc(alpha)+qpc(alpha))*qpc(gamma)*gqc(gamma)
      end if
      if (gamma==alpha) then
         term_ced=term_ced-(gqc(alpha)+qpc(alpha))*qpc(gamma)*gqc(idir)
      end if
   end do
end do

term_ced=-two_pi*term_ced / (gq2*g2*a_idir) 

!term_ced=-(gqr(idir)+qphon(idir))/gq2 + gqr(idir)/dot_product(gqr(:),gqr(:))
!write(*,'(a10,2e20.10e2)') 'TERM',term,term_ced


! AMSrev ]
! AMScom: note that re<->im because of the i factor in the formula
!           work1(re,ii)=-rhog(im,ii)*term*two_pi 
!           work1(im,ii)=rhog(re,ii)*term *two_pi

           work1(re,ii)=-rhog(im,ii)*term_ced*two_pi 
           work1(im,ii)=rhog(re,ii)*term_ced*two_pi

!AMSrev
!           write(212,'(9f20.12)') rhog(re,ii), rhog(im,ii), gqr(:), term, gs,gq2, gmet(1,1) 
         else
           work1(re,ii)=0.0_dp
           work1(im,ii)=0.0_dp
         end if
...
!Fourier Transform Vhartree.
!Vh in reciprocal space was stored in work1
 call fourdp(cplex,work1,vhartr1,1,mpi_enreg,nfft,1,ngfft,0)
 ABI_FREE(work1)

end subroutine hartremet
!!***
