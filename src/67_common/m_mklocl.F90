...
!!***
 ...
 public :: dfpt_vlocal_ams       ! CEDrev: AMS version of vlocal
!!***
...
!CEDrev: G0 pass zion
subroutine dfpt_vlocal_ams(atindx,cplexin,gmet,gsqcut,idir,ipert,&
& mpi_enreg,mqgrid,natom,nattyp,nfft,ngfft,&
& ntypat,n1,n2,n3,ph1d,qgrid,qphon,ucvol,vlspl,vpsp1,xred,zion)
...
!Arguments -------------------------------
!scalars
 integer,intent(in) :: cplexin,idir,ipert,mqgrid,n1,n2,n3,natom,nfft,ntypat !CEDrev: G0 cplexin
 real(dp),intent(in) :: gsqcut,ucvol
...
! AMSrev
 real(dp) :: sfr_G,sfi_G,vion1_G,gsquar_G,gmag_G,q2
 logical :: qeq0
!arrays
 integer, ABI_CONTIGUOUS pointer :: fftn2_distrib(:),ffti2_local(:)
 integer, ABI_CONTIGUOUS pointer :: fftn3_distrib(:),ffti3_local(:)
...
! AMSrev[
 write(*,*) 'ams: vloca3_ams: I am in. Idir=', idir
...
!AMSrev: summing on all the atoms 
  do iatom=1,natom
!  Determination of the atom type
   ia1=0
   itypat=0
...
!  AMSrev
           gsquar_G=gsq(gq1-qphon(1),gq2-qphon(2),gq3-qphon(3))
...
!            This bug fixed here 27 Jan 1992.)
             ...
             ! CEDrev: remove G0 (MS implementation)
             vion1 = (aa*vlspl(jj,1,itypat)+bb*vlspl(jj+1,1,itypat) + &
&             cc*vlspl(jj,2,itypat)+dd*vlspl(jj+1,2,itypat) + facg0) &
&             / gsquar
!             vion1 = (aa*vlspl(jj,1,itypat)+bb*vlspl(jj+1,1,itypat) + &
...
! AMSrev [
! Maybe is neede an if in order to check the lenght of gmag_G<cutoff?

! The metric extra term must be foced to zero when G=0 because otherwise spline returns NaN
!             if(gsquar_G<=1.d-15) then
             if(i1==1.and.i2==1.and.i3==1) then
               sfr_G=zero;sfi_G=zero
             else
           
               gmag_G=sqrt(gsquar_G)
               jj=1+int(gmag_G*dqm1)
               diff=gmag_G-qgrid(jj)
               ...
               ! CEDrev: remove G0 (MS implementation)
               vion1_G = (aa*vlspl(jj,1,itypat)+bb*vlspl(jj+1,1,itypat) + &
                    & cc*vlspl(jj,2,itypat)+dd*vlspl(jj+1,2,itypat) + facg0) &
                    & / gsquar_G
...
!&               / gsquar_G
               ...
               sfr_G=phimag(ig1,ig2,ig3,iatom)*2.0_dp*pi*(gq(idir)-qphon(idir))*vion1_G
               sfi_G=phre(ig1,ig2,ig3,iatom)*2.0_dp*pi*(gq(idir)-qphon(idir))*vion1_G
             end if
             
!AMSrev
!             write(222,*) sfr+sfr_G, sfi+sfi_G , vion1, vion1_G
!            Phase   q*xred  (complex conjugate)
             work1(re,ii)=work1(re,ii)+sfr+sfr_G!
             work1(im,ii)=work1(im,ii)+sfi+sfi_G!
! AMSrev ]

             !CEDrev: G0 Put facg0 to zero, otherwise the whole column will be affected
             facg0 = 0.d0
...
! AMSrev
  end do  ! on all the atoms (iatom)

!AMSrev[ moved to kpgmet (1st order kinetic subroutine) which is call in vtorho3
! add here the geom potential in case of a metric perturbation
!  q2=gsq(qphon(1),qphon(2),qphon(3))
!AMScom ucvol is here because later everithing is multiplied by 1/ucvol but the geom 
!pot should not have the volume factor
!  work1(im,:)=work1(im,:)-(2.0_dp*pi)**3/4.0_dp*qphon(idir)*q2*ucvol
!  
!AMSrev ]
...
end subroutine dfpt_vlocal_ams
